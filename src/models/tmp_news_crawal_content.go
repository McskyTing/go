...
type TmpNewsCrawalContent struct {
	...
	Extparamsjson string    `xorm:"not null default '' comment('额外变量') VARCHAR(2000)"`
	IsRepeat      int       `xorm:"not null default 0 comment('是否重复0-不重复，1-重复') TINYINT(1)"`
	RepeatTimes   int       `xorm:"not null default 0 comment('重复爬取次数') INT(11)"`
